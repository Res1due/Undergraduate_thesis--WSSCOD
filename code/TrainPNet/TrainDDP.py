...
from utils.utils import clip_gradient, adjust_lr, get_coef, cal_ual, get_weak_ratio_coef, weighted_mean
    ...
    for epoch in range(1, opt.epoch):
        ...
        for i, (fully_train, weak_train) in enumerate(zip(fully_train_loader, weak_train_loader), start=1):
            ...
            with amp.autocast(enabled=True):
                ...
                loss_init = weighted_mean(nc_loss(preds[0], gts, q_value), sample_weight) * 0.0625 + weighted_mean(nc_loss(preds[1], gts, q_value), sample_weight) * 0.125 + weighted_mean(nc_loss(preds[2], gts, q_value), sample_weight) * 0.25 + weighted_mean(nc_loss(preds[3], gts, q_value), sample_weight) * 0.5
                ...
                loss_final = weighted_mean(nc_loss(preds[4], gts, q_value), sample_weight)
            # loss_final = structure_loss(preds[4], gts)

                loss_edge = weighted_mean(dice_loss(preds[6], edges), sample_weight) * 0.125 + weighted_mean(dice_loss(preds[7], edges), sample_weight) * 0.25 + weighted_mean(dice_loss(preds[8], edges), sample_weight) * 0.5
            ...
            if rank == 0 and (i % 20 == 0 or i == total_step or i == 1):
                mean_weak_quality = weak_quality.mean().item()
                print(f'{datetime.now().strftime("%Y-%m-%d %H:%M:%S")} Epoch [{epoch:03d}/{opt.epoch:03d}], Step [{i:04d}/{total_step:04d}], LR {lr:.8f} Total_loss: {loss.item():.4f} Loss1: {loss_init.item():.4f} Loss2: {loss_final.item():.4f} Loss3: {loss_edge.item():.4f} Q: {q_value:.4f} WeakCoef: {weak_ratio_coef:.3f} WeakQuality: {mean_weak_quality:.3f} Size: {images.shape}')
                logging.info(f'[Train Info]:Epoch [{epoch}/{opt.epoch}], Step [{i}/{total_step}], Total_loss: {loss.item():.4f} Loss1: {loss_init.item():.4f} Loss2: {loss_final.item():.4f} Loss3: {loss_edge.item():.4f} WeakCoef: {weak_ratio_coef:.3f} WeakQuality: {mean_weak_quality:.3f}')
...
def main():
    ...
    parser.add_argument('--weak_train_root', type=str, default='../pseudo_label/ANet/CAMO_COD_generate_', help='the training rgb images root')
    ...
    parser.add_argument('--weak_weight', type=float, default=1.0, help='global weight for weakly supervised samples')
    parser.add_argument('--min_weak_weight', type=float, default=0.35, help='minimum sample weight for weak pseudo labels')
    parser.add_argument('--weak_warmup_epoch', type=int, default=15, help='epochs used to warm up weakly supervised samples')
